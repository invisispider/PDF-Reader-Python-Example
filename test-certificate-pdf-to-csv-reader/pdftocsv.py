...
# ##  Project/Python setup - install python 3 and pip 3.

# 1. extract zip to local coa_pfttocsv project folder.
# 2. inside folder, 'python3 venv myenv'   to create an environment named "myenv"
# 3. 'source myenv/bin/activate'. You should see (myenv) in shell.



# ## Do not continue without the environment created and activated.
# ## Do not customize the names of anything unless you know what you're doing.

# With environment activated, in main project folder, pip3 install -r requirements.txt
# This should install all required packages into environment. Note: I used python@3.1


## ! create folders "output" and "input" (case-sensitive) and set permissions so that python can rwx in these folders.

# Place all COA.pdf files into the input folder.

# with environment activated and in the project folder,
# python3 coa_pdftocsv.py


# should be able to convert any CAMBIUM or VIRIDIS COA pdfs from input folder and convert them to dataframe.csv in output folder
...
def get_data_cambium(coa):
    ...
    for line in coa:
        ...
        # ANALYTE AMT (%) LOD/LOQ (µg/g) PASS/FAIL ANALYTE AMT (%) LOD/LOQ (µg/g) PASS/FAIL
        # TOTAL TERPENES * 6.732 % N/A CARYOPHYLLENE OXIDE 0.024 % 0.5/1 N/A
